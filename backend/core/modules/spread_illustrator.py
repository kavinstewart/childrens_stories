...
class SpreadIllustrator:
    ...
    def _build_contents(
        ...
        if reference_sheets and reference_sheets.character_sheets:
            ...
            # Debug logging to diagnose character reference issues
            print(
                f"DEBUG Spread {spread.spread_number}: present_characters={spread.present_characters}, "
                f"resolved_characters={characters_in_spread}",
                file=sys.stderr
            )
            ...
            for char_name in characters_in_spread:
                ...
                if sheet and bible:
                    ...
                    # Identity locking: emphatic instruction to maintain exact character appearance
                    # Using strong, direct language to maximize model compliance
                    contents.append(
                        f"CRITICAL CHARACTER REFERENCE - {bible.name.upper()}: "
                        f"The character {bible.name} in this scene MUST look EXACTLY like the reference image above. "
                        f"REQUIRED: Same face shape, same hair color ({bible.hair or 'as shown'}), same age ({bible.age_appearance or 'as shown'}), "
                        f"same clothing ({bible.clothing or 'as shown'}). "
                        f"DO NOT change the character's appearance. DO NOT make them younger or older. "
                        f"The reference image is the ONLY correct depiction of {bible.name}. "
                        f"Full description: {bible.to_prompt_string()}"
                    )
                    added_refs += 1
                    print(
                        f"DEBUG Spread {spread.spread_number}: Added reference for '{bible.name}'",
                        file=sys.stderr
                    )
                else:
                    print(
                        f"DEBUG Spread {spread.spread_number}: Could not find sheet/bible for '{char_name}' "
                        f"(sheet={sheet is not None}, bible={bible is not None})",
                        file=sys.stderr
                    )
