...
class LogEntry(BaseModel):
    ...
    # Context limited to 10 keys with string values only (prevents arbitrary nested dicts)
    context: Optional[dict[str, str]] = Field(default=None, max_length=10)
...
async def ingest_logs(request: IngestLogsRequest, user: CurrentUser):
    ...
    if request.device_id:
        device_info = f" device={sanitize_log_message(request.device_id)}"
    if request.session_id:
        device_info += f" session={sanitize_log_message(request.session_id)}"

    for entry in request.entries:
        context_str = ""
        if entry.context:
            # Sanitize each context value
            safe_context = {k: sanitize_log_message(v) for k, v in entry.context.items()}
            context_str = f" {safe_context}"

        safe_message = sanitize_log_message(entry.message)
        safe_timestamp = sanitize_log_message(entry.timestamp)
        log_message = f"{device_info} [{safe_timestamp}] {safe_message}{context_str}"
