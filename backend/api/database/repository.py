...
class StoryRepository:
    ...
    def _build_spread_responses(
        ...
        Args:
            ...
            spreads: List of spread records from database
            metadata: Story metadata with illustration style
            story_status: Story status for computing illustration_status
        """
        style = metadata.illustration_style if metadata else None

        def get_illustration_status(has_illustration: bool) -> str:
            """Determine illustration status based on presence and story state."""
            if has_illustration:
                return "complete"
            # No illustration - check if story is done (completed or failed)
        ...
        return [
            StorySpreadResponse(
                ...
                illustration_status=get_illustration_status(bool(s["illustration_path"])),
    ...
    def _record_to_response(
        ...
        spread_responses = (
            self._build_spread_responses(
                story["id"], spreads, metadata, story_status=story["status"]
            )
