...
describe('useRegenerateSpread', () => {
  ...
  describe('loading states', () => {
    ...
    it('returns regeneratingSpreadNumber while regeneration is in progress', async () => {
      const { queryClient, wrapper } = createWrapper();
      const mockStory = createMockStory();

      queryClient.setQueryData(storyKeys.detail('test-story-id'), mockStory);

      // Use slow mock so we can check intermediate state
      let resolveApi: (value: any) => void;
      mockedApi.regenerateSpread.mockImplementation(
        () =>
          new Promise((resolve) => {
      ...
      // Initially no spread is regenerating
      expect(result.current.regeneratingSpreadNumber).toBeNull();
      ...
      // While mutation is in progress, should track the spread number
      await waitFor(() => {
        expect(result.current.regeneratingSpreadNumber).toBe(2);
      });
      ...
      // After success, regeneratingSpreadNumber should STILL be set
      // (because actual regeneration is async and takes time)
      // Caller must use resetRegenerating() when they detect completion
      await waitFor(() => {
        expect(result.current.regeneratingSpreadNumber).toBe(2);
      });
    });

    it('provides resetRegenerating to clear state when caller detects completion', async () => {
      const { queryClient, wrapper } = createWrapper();
      const mockStory = createMockStory();
      ...
      // State should still be set after success
      expect(result.current.regeneratingSpreadNumber).toBe(2);

      // Caller clears state when they detect regeneration is complete
      act(() => {
        result.current.resetRegenerating();
      });
