...
describe('useSTT no-activity timeout (safety net)', () => {
  beforeEach(() => {
    jest.useFakeTimers();
    mockWsInstance = null;
    wsOpenCallback = null;
  ...
  it('fires onNoActivityTimeout when speech_started but no utterance_end arrives', async () => {
    const onNoActivityTimeout = jest.fn();
    const onSilenceTimeout = jest.fn();
    ...
    // Noise triggers speech_started but NO utterance_end follows
    act(() => {
      mockWsInstance?.simulateMessage({ type: 'speech_started' });
    });

    // Wait some time - silence timeout should NOT fire (no utterance_end)
    await act(async () => {
      await jest.advanceTimersByTimeAsync(5000);
    });

    expect(onSilenceTimeout).not.toHaveBeenCalled();
    expect(onNoActivityTimeout).not.toHaveBeenCalled();

    // Wait until no-activity timeout fires
    await act(async () => {
      await jest.advanceTimersByTimeAsync(5000);
    });

    expect(onNoActivityTimeout).toHaveBeenCalledTimes(1);
    expect(onSilenceTimeout).not.toHaveBeenCalled();
  });

  it('clears no-activity timeout when utterance_end is received', async () => {
    const onNoActivityTimeout = jest.fn();
    const onSilenceTimeout = jest.fn();
    ...
    // Wait 5s (halfway through no-activity timeout)
    await act(async () => {
      await jest.advanceTimersByTimeAsync(5000);
    });

    // Normal speech flow - utterance_end clears the no-activity timeout
    act(() => {
      mockWsInstance?.simulateMessage({ type: 'utterance_end' });
    });

    // Wait past original no-activity timeout
    await act(async () => {
      await jest.advanceTimersByTimeAsync(6000);
    });

    // No-activity timeout should NOT fire (was cleared)
    expect(onNoActivityTimeout).not.toHaveBeenCalled();
    // But silence timeout SHOULD fire (3s after utterance_end, we waited 6s)
    expect(onSilenceTimeout).toHaveBeenCalledTimes(1);
  });

  it('does not fire no-activity timeout when disabled (0ms)', async () => {
    const onNoActivityTimeout = jest.fn();
