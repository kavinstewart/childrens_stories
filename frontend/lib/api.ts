...
export interface StorySpread {
  ...
  /**
   * Status of the illustration for this spread:
   * - "complete": illustration exists
   * - "failed": story completed but illustration generation failed
   * - "pending": illustration still being generated
   */
...
export const api = {
  ...
  listStories: async (params?: {
    ...
    return fetchApi<StoryListResponse>(`/stories/${query ? `?${query}` : ''}`);
  },
  ...
  getRecommendations: async (storyId: string, limit: number = 3): Promise<RecommendationsResponse> => {
