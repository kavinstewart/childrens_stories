...
const NO_ACTIVITY_TIMEOUT_MS = 15000; // 15 second safety net if utterance_end never arrives
const INTERIM_DEBOUNCE_MS = 150; // Debounce interim transcript updates

/** Append text to accumulated transcript with space separator */
const appendText = (accumulated: string, newText: string): string =>
  accumulated ? `${accumulated} ${newText}` : newText;
...
export default function NewVoiceStory() {
  ...
  // Helper to clear debounce timer (DRY)
  ...
  // Helper to reset transcript state for new recording (DRY)
    ...
    if (data.isFinal) {
      ...
      if (!debounceTimerRef.current) {
        debounceTimerRef.current = setTimeout(() => {
          setTranscript(appendText(transcriptRef.current, interimRef.current));
          debounceTimerRef.current = null;
        }, INTERIM_DEBOUNCE_MS);
      }
    }
  ...
  // Handle no-activity timeout (safety net for when utterance_end never arrives)
  const handleNoActivityTimeout = useCallback(() => {
    console.log('[Voice] No-activity timeout triggered - resetting');
    setError('No speech detected. Please tap the mic and try again.');
    setVoiceState('idle');
  }, []);
  ...
  const clearDebounceRef = useRef(clearDebounce);
